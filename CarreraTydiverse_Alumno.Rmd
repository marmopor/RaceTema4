...
#Convertir el df a un tibble:
cran <- as_tibble(mydf)

#Borramos el df original:
rm(mydf)
```
...
```{r}
#Visualización directa del tibble "cran":
cran

#Utilizando la función glimpse sobre "cran"
glimpse(cran)
```
# dplyr
...
#Selección de variables utilizando dplyr:
cran_select <- select(cran, ip_id, package, country)
```
...
#Selección de variables utilizando dplyr:
cran_select <- select(cran, r_arch, r_os, package, version, country)
```
...
#Selección de todas las columnas excepto las que están entre "date" y "size":
cran_select <- select(cran, -date:size)
```
...
filtered_cran <- filter(cran, package == "plotly" & r_version == "3.5.1")
```
...
#Filtrar los registros:
filtered_cran <- filter(cran, country == "ES" & as.numeric(strsplit(r_version, "\\.")[[1]]) > 3 & as.numeric(strsplit(r_version, "\\.")[[1]])[2] > 4)
```
...
#Filtrar los registros:
filtered_cran <- filter(cran, country %in% c("ES", "PT"))
```
...
#Filtrar los registros:
filtered_cran <- filter(cran, r_os == "linux-gnu" & size > 100500)
...
#Filtrar los registros:
filtered_cran <- filter(cran, r_os != "NA" & size > 100500)
```
10. Almacena en `cran2` todas las columnas entre `size` y `ip_id`. 
```{r}
library(dplyr)

#Almacenar todas las columnas entre 'size' e 'ip_id' en cran2:
...
#Ordenar cran2 en orden ascendente: 
...
#Ordenar cran2 en orden descendente: 
...
#Ordenar cran2 por package e ip_id:
...
#Ordenar cran2 según country (ascendente), r_version (descendente) e ip_id (ascendente)
...
#Seleccionar las variables ip_id, package y size en ese orden:
...
#Crear la variable size_gb:
cran3 <- mutate(cran3, size_gb = size / (2^20) / (2^10))
```
...
#Crear la variable correct_size:
...
#Calcular el valor medio de la variable size sobre el element. agrupado:
avg_size_by_package <- summarise(by_package, avg_size = mean(size))
```
...
#Calcular el umbral para seleccionar el 1% de los paquetes más descargados:
  umbral <- cran %>%
  count(package) %>%
  summarise(umbral = quantile(n, probs = 0.99))

#Seleccionar los paquetes más descargados (top_counts):
  top_counts <- cran %>%
  count(package) %>%
  filter(n >= umbral$umbral)
```
...
#Calcular el umbral igual que anteriormente:
  umbral <- cran %>%
  count(package) %>%
  summarise(umbral = quantile(n, probs = 0.99))

#Seleccionar los paquetes más descargados (top_counts) 
#y ordenar en orden descendente:
...
#Calcular el umbral por número de ordenadores distintos:
  umbral_unique <- cran %>%
  count(package, ip_id) %>%
  count(package) %>%
  summarise(umbral_unique = quantile(n, probs = 0.99))

#Seleccionar los paquetes más descargados por número de ordenadores distintos
#(top_unique) y ordenar en orden descendente:
  top_unique_sorted <- cran %>%
  count(package, ip_id) %>%
  count(package) %>%
  filter(n >= umbral_unique$umbral_unique) %>%
  arrange(desc(n))
```
...
#Seleccionar las columnas ip_id, country, package, size: 
...
#Añadir la columna size_mb: 
  add <- cran %>% select(ip_id, country, package, size) %>%
...
#Seleccionar las descargas por tam.:
anterior <- add %>% filter(size_mb <= 0.5)
```
...
#Seleccionar y ordenar las descargas en orden descendente:
anterior %>% filter(size_mb <= 0.5) %>% arrange(desc(size_mb)) 
```
...
```{r}
# Cargamos el fichero:
data <- load(file ="data/datostidyr.Rdata")
head (data)
summary (data)
```
...
#grade: La calificación.
#male: El número de estudiantes hombres.
#female: El número de estudiantes mujeres.
```

Transfórmalo en un conjunto `tidy` con las variables, `grade, sex y count`.
```{r}
library(tidyverse)
